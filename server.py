...
def handle_api_request():
    ...
    Trả về dạng JSON, các key không để dấu, nếu trường tổng số tiền không có thông tin thì trả về "không đủ thông tin",ghi chú nội dung hóa đơn bằng vài từ ngắn gon, chú ý: định dạng lại thời gian dạng **/**/****, đặc biệt: chỉ trả lời chuỗi json theo yêu cầu không thêm bớt nội dung.
    """
        ...
        total_amount = text.get('tong tien thanh toan', '0').replace('.', '').replace(',', '')
        ...
        try:
            ...
            # Nếu không có user_name trùng lặp, thực hiện thêm mới người dùng
            insert_query = """
        ...
        try:
            ...
            if user:
                # print("vô")
                select_user = """select amount, user_email, user_name from users where user_id = %s"""
        ...
        try:
            ...
            insert_query = """
                    ...
                    VALUES (%s, %s, %s, %s)
                    """
            cursor.execute(insert_query, (icon_name, categories_name, user_id, categories_type))
            conn.commit()
